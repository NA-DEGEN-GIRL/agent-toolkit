...
Use this Codex-specific skill for requested file-based work checkpoints and handoffs. Snapshots live in the target repo at `.handoff/latest.md` plus dated backups; saving one does not require clearing or leaving the current session.
...
- **Save Mode:** the user requests a persisted work snapshot/checkpoint or a file-based handoff for another session/compatible agent. Examples: `handoff 저장해줘`, `이 대화는 계속할 건데 현재 상태를 체크포인트 파일로 남겨줘`, `Claude로 넘길 인수인계 파일 만들어줘`. Honor the request without also requiring a reset or transfer.
- **Resume Mode:** the user requests continuation from a saved handoff, by naming the skill with resume intent, a snapshot/lane, or a clearly identified file-based handoff. Examples: `use codex-handoff, 이어받아`, `.handoff/latest.md 검증하고 계속해`, `auth-refactor scope handoff 이어받아`.
...
- Use a Codex handoff skill in Save Mode; do not require or recommend a reset unless the user chose one.
